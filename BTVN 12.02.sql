...
Where tongTien <= 300000 AND theLoaiPhim.tenTheLoaiPhim != 'Hành động';

-- Lấy ra 3 người thuê nhiều phim nhất.Thông tin những người thuê phim đó ?
SELECT User.ID_User, User.hoTen, User.SDT, User.gioiTinh, count(User.ID_User) as soLuongPhim FROM thuePhim
INNER JOIN User ON User.ID_User = thuePhim.ID_User GROUP BY User.ID_User ORDER BY soLuongPhim DESC LIMIT 3;

-- Tính tổng doanh thu số tiền của một bộ phim trong 2 ngày
SELECT ID_Phim, thoiGianBatDau, SUM(tongTien) FROM thuePhim WHERE thoiGianBatDau = '2023-01-15' AND ID_Phim ='P1';
 
-- Lấy ra top 3 bộ phim được xem nhiều nhất trong 1 ngày
SELECT Phim.ID_Phim, Phim.tenPhim, theLoaiPhim.tenTheLoaiPhim, danhMucPhim.tenDanhMuc, thuePhim.thoigianBatDau, count(Phim.ID_Phim)
as soLuongThue FROM thuePhim
INNER JOIN Phim ON thuePhim.ID_Phim = Phim.ID_Phim
INNER JOIN theLoaiPhim ON Phim.ID_theLoaiPhim = theLoaiPhim.ID_theLoaiPhim
INNER JOIN danhMucPhim ON Phim.ID_DanhMuc = danhMucPhim.ID_DanhMuc  
WHERE thoiGianBatDau = '2023-01-15'
GROUP BY Phim.ID_Phim ORDER BY soLuongThue DESC LIMIT 3;

-- Hiển thị danh sách tất cả thông tin của phim
SELECT Phim.ID_Phim, Phim.tenPhim, theLoaiPhim.tenTheLoaiPhim, danhMucPhim.tenDanhMuc, Phim.ngaySanXuan, Phim.giaPhim
FROM Phim INNER JOIN theLoaiPhim ON Phim.ID_theLoaiPhim = theLoaiPhim.ID_theLoaiPhim
INNER JOIN danhMucPhim ON Phim.ID_DanhMuc = danhMucPhim.ID_DanhMuc
